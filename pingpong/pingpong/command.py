...
# symbols which are imported by "from pingpong.command import *"
...
try:
    ...
    print """\
This is a TurboGears 1.5 (http://www.turbogears.org) application.
...
from pingpong.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('pingpong'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='pingpong.config')
...
def bootstrap():
    ...
    By default this runs the function 'pingpong.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-pingpong = pingpong.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="pingpong %s" % version)
    ...
    _read_config(args)
    from pingpong.model import bootstrap_model
...
def start():
    ...
    from pingpong.controllers import Root
